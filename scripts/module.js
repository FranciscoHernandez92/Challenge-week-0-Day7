...
export const game = () => {
  ...
  while (playAgain === true) {
    ...
    document.getElementById('userCard').innerText = `**${username}** Tu carta es ${userCard.numero} de ${userCard.symbol}`;
    ...
    if (!cards.length) {
      document.getElementById('endMessage').innerText = "Se acabaron las cartas!";
      break;
    }
    playAgain = document.getElementById('playAgain').checked;
  }

  document.getElementById('finalMessage').innerText = `Gracias por haber jugado **${username}** Has jugado ${match} vez y ganado ${score} vez!`;
};

document.addEventListener('DOMContentLoaded', (event) => {
  document.getElementById('startButton').addEventListener('click', game);
});
